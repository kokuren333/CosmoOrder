...
import { HistoryPanel, ProgressPanel } from "./components/ProgressPanel.tsx";
...
function DiagnosticList({ diagnostics }: { diagnostics: ErrorView[] }): ReactElement {
  return (
    <ul className="diagnostics">
      {diagnostics.map((diagnostic, index) => (
        <li key={`${diagnostic.code}-${index}`}>
          <strong>{diagnostic.code}</strong> {diagnostic.message}
          {diagnostic.file === null ? null : <> <code>{diagnostic.file}</code></>}
...
export function App(): ReactElement {
  ...
  const openPackage = useCallback(
    async (packageId: string) => {
      ...
      try {
        ...
        setAttempt(null);
        setEvents([]);
        setProgress(await osmium.progress(loaded.package_id, loaded.package_version));
  ...
  const openResource = useCallback(
    async (resourceId: string) => {
      ...
      setBusy(true);
      try {
        setResource(await osmium.readResource(lesson.package_id, resourceId, lesson.package_version));
  ...
  const answer = useCallback(
    async (assessmentId: string, response: string | boolean) => {
      ...
      try {
        ...
        setAttempt(result);
        setProgress(await osmium.progress(lesson.package_id, lesson.package_version));
        setDiagnostics([]);
      } catch (error) {
        fail(error);
      } finally {
  ...
  const order: Resource[] = useMemo(() => (lesson === null ? [] : readingOrder(lesson)), [lesson]);

  const selectedResource =
    panel.name === "reader" ? (order.find((item) => item.id === panel.resourceId) ?? null) : null;
  const resourceIndex =
    selectedResource === null ? -1 : order.findIndex((item) => item.id === selectedResource.id);
  const previous = resourceIndex > 0 ? (order[resourceIndex - 1] ?? null) : null;
  const next =
  ...
  const currentAssessment =
    panel.name === "assessment"
      ? (lesson?.assessments.find((item) => item.id === panel.assessmentId) ?? null)
      : null;
  const assessmentIndex =
    currentAssessment === null
      ? -1
      : (lesson?.assessments.findIndex((item) => item.id === currentAssessment.id) ?? -1);
  const nextAssessment =
    lesson !== null && assessmentIndex >= 0 && assessmentIndex < lesson.assessments.length - 1
    ...
    <div className="app" style={{ fontSize: `${scale}rem` }}>
      <header className="app-header">
        ...
        <div className="row">
          ...
          <button type="button" onClick={() => setScale(1.5)} disabled={scale === 1.5}>
            文字 150%
          </button>
          <button type="button" onClick={() => setScale(2)} disabled={scale === 2}>
            文字 200%
          </button>
        </div>
      </header>
      ...
      {status === null ? null : (
        <p className="meta app-status">
          データ: <code>{status.home}</code> · DB: <code>{status.database}</code> · state
      ...
      <main>
        ...
        {lesson !== null && panel.name === "lesson" ? (
          <Lesson
            ...
            onShowHistory={() => void showHistory(0)}
          />
        ) : null}
